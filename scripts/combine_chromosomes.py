...
def main(intermediate_dir, hatchet1_dir, hatchet2_dir):
    ...
    totalcounts.to_csv(os.path.join(hatchet1_dir, 'rdr', 'total.tsv'), header = False)
    totalcounts.to_csv(os.path.join(hatchet2_dir, 'rdr', 'total.tsv'), header = False)

    joint_tumor_baf = pd.concat([pd.read_table(os.path.join(intermediate_dir, 'baf', f)) for f in baf_files if 'normal' not in f])[['chr', 'pos', 'sample', 'ref', 'alt']]
    joint_normal_baf = pd.concat([pd.read_table(os.path.join(intermediate_dir, 'baf', f)) for f in baf_files if 'normal' in f])[['chr', 'pos', 'sample', 'ref', 'alt']]

    joint_tumor_baf.to_csv(os.path.join(hatchet1_dir, 'baf', 'tumor.1bed'), index = False, header = False)
    joint_normal_baf.to_csv(os.path.join(hatchet1_dir, 'baf', 'normal.1bed'), index = False, header = False)
    joint_tumor_baf.to_csv(os.path.join(hatchet2_dir, 'baf', 'tumor.1bed'), index = False, header = False)
    joint_normal_baf.to_csv(os.path.join(hatchet2_dir, 'baf', 'normal.1bed'), index = False, header = False)
    ...
    joint_tumor_rdr.to_csv(os.path.join(hatchet1_dir, 'rdr', 'tumor.1bed'), index = False, header = False)
    joint_normal_rdr.to_csv(os.path.join(hatchet1_dir, 'rdr', 'normal.1bed'), index = False, header = False)
